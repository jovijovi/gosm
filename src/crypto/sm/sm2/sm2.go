...
func ZA(pub *PublicKey, uid []byte) ([]byte, error) {
	...
	za.Write(uid)
	za.Write(elliptic.SM2P256ToBig(&elliptic.SM2P256.SMa).Bytes())
	za.Write(elliptic.SM2P256.B.Bytes())
	za.Write(elliptic.SM2P256.Gx.Bytes())
	za.Write(elliptic.SM2P256.Gy.Bytes())
...
func Decompress(a []byte) *PublicKey {
	var aa, xx, xx3 elliptic.SM2P256FieldElement

	elliptic.P256Sm2()
	x := new(big.Int).SetBytes(a[1:])
	curve := elliptic.SM2P256
	elliptic.SM2P256FromBig(&xx, x)
	elliptic.SM2P256Square(&xx3, &xx)       // x3 = x ^ 2
	elliptic.SM2P256Mul(&xx3, &xx3, &xx)    // x3 = x ^ 2 * x
	elliptic.SM2P256Mul(&aa, &curve.SMa, &xx) // a = a * x
	elliptic.SM2P256Add(&xx3, &xx3, &aa)
	elliptic.SM2P256Add(&xx3, &xx3, &curve.SMb)

	y2 := elliptic.SM2P256ToBig(&xx3)
	y := new(big.Int).ModSqrt(y2, elliptic.SM2P256.P)
	if getLastBit(y) != uint(a[0]) {
		y.Sub(elliptic.SM2P256.P, y)
	}
	return &PublicKey{
