...
func ParsePKCS8Sm2PrivateKey(der, pwd []byte) (*sm.PrivateKey, error) {
	if pwd == nil {
...
func ReadPrivateKeyFromMem(data []byte, pwd []byte) (*sm.PrivateKey, error) {
	var block *pem.Block
	...
	priv, err := ParsePKCS8Sm2PrivateKey(block.Bytes, pwd)
	return priv, err
}

func ReadPrivateKeyFromPem(FileName string, pwd []byte) (*sm.PrivateKey, error) {
...
func WritePrivateKeytoMem(key *sm.PrivateKey, pwd []byte) ([]byte, error) {
	var block *pem.Block
...
func WritePrivateKeytoPem(FileName string, key *sm.PrivateKey, pwd []byte) (bool, error) {
	var block *pem.Block
...
func ReadPublicKeyFromMem(data []byte, _ []byte) (*sm.PublicKey, error) {
	block, _ := pem.Decode(data)
...
func ReadPublicKeyFromPem(FileName string, pwd []byte) (*sm.PublicKey, error) {
	data, err := ioutil.ReadFile(FileName)
	if err != nil {
		return nil, err
	}
...
func WritePublicKeytoMem(key *sm.PublicKey, _ []byte) ([]byte, error) {
...
func WritePublicKeytoPem(FileName string, key *sm.PublicKey, _ []byte) (bool, error) {
	der, err := MarshalSm2PublicKey(key)
	if err != nil {
		return false, err
	}
